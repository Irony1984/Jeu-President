...
const TransferManager = (function () {
  ...
  /* --- NÉGOCIATION D'ACHAT (double : club puis joueur) ---
     ...
     PATIENCE : le club vendeur se lasse d'un marchandage à la baisse.
     Chaque offre refusée l'agace (+1) ; proposer MOINS que la fois
     précédente est un affront (+2). Passé un seuil, il claque la porte
     et rompt définitivement les négociations (offer.status = "broken"). */
  const CLUB_PATIENCE = 3;
  ...
  function negotiateClub(offer, proposedPrice) {
    ...
    // Offre insuffisante : le club vendeur s'agace.
    if (offer.clubAnger === undefined) offer.clubAnger = 0;
    const loweredBid = offer.lastProposed !== undefined && proposedPrice < offer.lastProposed;
    offer.lastProposed = proposedPrice;
    offer.clubAnger += loweredBid ? 2 : 1;

    // Patience épuisée : rupture des négociations.
    if (offer.clubAnger >= CLUB_PATIENCE) {
      offer.status = "broken";
      return { ok: false, broken: true, loweredBid };
    }

    // Sinon, contre-proposition à mi-chemin, avec humeur montante.
    const counter = _round((proposedPrice + ask) / 2);
    const annoyed = offer.clubAnger >= CLUB_PATIENCE - 1;
    return { ok: false, counter, annoyed, loweredBid, triesLeft: CLUB_PATIENCE - offer.clubAnger };
  }
