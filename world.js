...
(function () {
    ...
    showNegotiation(
      ...
      choices.map(c => ({
        ...
        onPick: () => {
          ...
          if (r.ok) { toast(`Accord trouvé à ${r.agreedPrice} M !`); saveAndRefreshTransfers(); }
          else { toast(`Refusé (le club veut ${r.counter} M). Réessayez.`); }
        },
      }))
    );
  }
  ...
  function renderStaff() {
    ...
    for (const role of StaffManager.ROLES) {
      const cur = StaffManager.current(gs, role);
      const block = document.createElement("div");
      block.className = "staff-block";
      const vac = cur.interim
        ? `<span class="staff-vacant">Poste vacant — ${cur.name} (${stars(cur.stars)})</span>`
        : `<span class="staff-holder">${cur.name} · ${stars(cur.stars)} · ${cur.salary} M/an</span>`;
      const styleLine = (role === "coach" && cur.style)
        ? `<div class="staff-cur">Style : <b>${StaffManager.STYLE_LABEL[cur.style]}</b></div>` : "";
      block.innerHTML =
        `<div class="staff-role">${StaffManager.ROLE_LABEL[role]}</div>` +
        `<div class="staff-cur">${vac}</div>` + styleLine;
      const actions = document.createElement("div");
      actions.className = "tr-actions";
      const bHire = document.createElement("button");
      bHire.className = "primary small";
      bHire.textContent = cur.interim ? "Recruter" : "Remplacer";
      bHire.onclick = () => openStaffHire(role);
      actions.appendChild(bHire);
      if (!cur.interim) {
        const bFire = document.createElement("button");
        bFire.className = "ghost small";
        bFire.textContent = "Licencier";
        bFire.onclick = () => {
          StaffManager.dismiss(gs, role);
          Storage.saveSlot(GameEngine.getSlot(), gs).catch(e => console.error(e));
          renderStaff();
        };
  ...
  /* Panneau de recrutement d'un poste : liste de candidats à comparer. */
  function openStaffHire(role) {
    const gs = GameEngine.getState();
  ...
  function renderSquad() {
    ...
    for (const p of squad) {
      ...
      const coach = StaffManager.current(gs, "coach");
